package com.odbpo.fenggou.data.api.cache;
...
public final class CacheManager {
    ...
    public static CacheManager getInstance(Application application) {
        if (mCacheManager == null) {
            synchronized (CacheManager.class) {
                if (mCacheManager == null) {
                    mCacheManager = new CacheManager(application);
                }
            }
        }
    ...
    private CacheManager(Application application) {
        File diskCacheDir = getDiskCacheDir(application, CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(application), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
