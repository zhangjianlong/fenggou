...
import com.core.op.lib.weight.imgselector.bean.Image;
...
public class MultiImageSelectorFragment extends Fragment {
    ...
    private int cropWight = 100;

    private int cropHeight = 100;

    private File mTmpFile;

    private File mTmpCropFile;
    ...
    private void showCameraAction() {
        if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
                try {
                    mTmpFile = FileUtils.createTmpFile(getActivity(), FileUtils.JPEG_FILE_PREFIX);
                } catch (IOException e) {
                    e.printStackTrace();
                }
...
//                }
    ...
    private boolean isCrop() {
        return getArguments() == null || getArguments().getBoolean(EXTRA_IS_CROP, false);
    }
